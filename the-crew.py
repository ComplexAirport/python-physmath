...
class Captain(Crew):
    def fire_lasers(self, ship, target_ship, target_location):
        if target_location not in Crew.locations:
            print('Not a valid location')
            ...
            for member in target_ship.crew_list:
...
class Doctor(Crew):
    ...
    def first_aid(self, ship):
        ...
        if self.medpacs < 0:
            ...
            for member in ship.crew_list:
                if member.location == self.location and member.status == 'Injured':
                    print(f'{self.name} healed {member.name}\'s injuries.')
                    member.status = 'Active'


class Starship:
    def __init__(self, name: str, crew_list: list[Crew]):
        self.name = name
        self.crew_list = crew_list
        self.damaged: dict[str, bool] = {value: False for value in Crew.locations}
